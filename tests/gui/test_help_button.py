...
class TestHelpTooltipSizing:
    ...
    def test_minimum_size_compliant(self) -> None:
        source = (
            ...
            / "src" / "omnicrawler" / "gui" / "widgets" / "help_tooltip.py"
...
class TestHelpRegistry:
    ...
    def test_search_finds_keywords(self) -> None:
        from omnicrawler.services.help_registry import search_help
        results = search_help("网址 URL")
        assert len(results) > 0
        assert any("seed" in r.help_id for r in results)

    def test_all_help_ids_in_registry(self) -> None:
        from omnicrawler.services.help_registry import NON_OBVIOUS_CONTROL_HELP_IDS
...
class TestCapabilityDisplay:
    ...
    def test_summary_text_includes_all_tiers(self) -> None:
        from omnicrawler.core.capabilities import capability_report, capability_summary_text
        report = capability_report()
    ...
    def test_individual_capability_structure(self) -> None:
        from omnicrawler.core.capabilities import _cap_item
...
class TestFieldSpecs:
    ...
    def test_all_required_fields(self) -> None:
        from omnicrawler.services.field_spec import FIELD_SPECS
    ...
    def test_help_html_contains_structure(self) -> None:
        from omnicrawler.services.field_spec import field_help_html
    ...
    def test_all_ids_listed(self) -> None:
        from omnicrawler.services.field_spec import all_field_ids
        ids = all_field_ids()
...
class TestAISafety:
    def test_untrusted_marker(self) -> None:
        from omnicrawler.services.ai_safety import mark_untrusted
    ...
    def test_validate_ai_output_rejects_unknown(self) -> None:
        from omnicrawler.services.ai_safety import validate_ai_output
