...
def test_image_only_pdf_runs_real_ocr_and_extracts(tmp_path: Path) -> None:
    ...
    # 2026-09-13：OCR 会把千分位逗号读成小数点（实测 12,345.67 → 12.345.67）。
    # 旧实现的正则只匹配到 `12.345` —— 一个**看起来合理却错误**的值，这正是本用例
    # 此前只敢断言"金额被识别出来了"的原因。归一改为**结构判定**后该形态可被唯一恢复，
    # 于是这里升级为与真值**逐字符相等**：证据从"识别出来了"变成"值是对的"。
    assert rows[0]["金额"] == AMOUNT_NORMALIZED, f"金额应恢复为真值：{rows[0]}"
    # 恢复必须可审计：原始 OCR 文本仍留在"…_原始值"里。
    assert rows[0]["金额_原始值"], "原始值必须保留（否则恢复无从复核）"
