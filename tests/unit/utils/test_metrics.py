...
def test_metrics_write_json_and_prometheus(tmp_path) -> None:
    ...
    metrics.increment("omnicrawler_failures_total", stage="parse", error='bad"value')
    metrics.gauge("omnicrawler_frontier_pending", 3)
    ...
    assert 'omnicrawler_request_duration_seconds_max{host="example.org"} 0.250000' in prometheus
    assert "omnicrawler_frontier_pending 3.0" in prometheus
    assert 'omnicrawler_stage_duration_seconds_p95{stage="crawl"} 0.480000' in prometheus
