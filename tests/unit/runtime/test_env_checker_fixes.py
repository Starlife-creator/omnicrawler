...
def test_f30_frozen_bundled_survives_timeout(monkeypatch) -> None:
    ...
    monkeypatch.setattr("omnicrawl.gui.runner.env_checker.resolve_cli_command", lambda p: p)
    monkeypatch.setattr("omnicrawl.core.runtime_paths.bundled_cli_path", lambda: Path("C:/app/omnicrawl.exe"))
    ...
    monkeypatch.setattr("omnicrawl.gui.runner.env_checker.subprocess.run", _raise_timeout)
    available, version = check_omnicrawl("C:/app/omnicrawl.exe")
...
def test_f30_frozen_bundled_survives_nonzero_rc(monkeypatch) -> None:
    ...
    monkeypatch.setattr("omnicrawl.core.runtime_paths.is_frozen", lambda: True)
    monkeypatch.setattr(
        "omnicrawl.gui.runner.env_checker.subprocess.run",
        lambda *a, **k: _Result(returncode=1, stdout=""),
    )
    available, _version = check_omnicrawl("C:/app/omnicrawl.exe")
    assert available is True
...
def test_f31_errors_classified_not_frozen(monkeypatch) -> None:
    ...
    monkeypatch.setattr("omnicrawl.gui.runner.env_checker.resolve_cli_command", lambda p: p)
    monkeypatch.setattr("omnicrawl.core.runtime_paths.bundled_cli_path", lambda: None)
    ...
    assert check_omnicrawl("omnicrawl") == (False, "")
...
def test_f35_derive_worker_command(tmp_path: Path) -> None:
    assert _derive_worker_command("omnicrawl") is None
    ...
    worker = tmp_path / "omnicrawl-worker.exe"
