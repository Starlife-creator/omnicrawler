...
def test_sink_errors_are_bounded_but_counts_remain_complete() -> None:
    manager = RecordSinkManager(sinks=[("mirror", _FailingSink())], max_errors=2, fail_open=True)
...
def test_s2516_default_fail_closed_raises_on_sink_error() -> None:
...
def test_s2516_default_config_is_fail_closed(tmp_path) -> None:
    config = AppConfig(
    ...
    manager = build_record_sink_manager(config)
    assert manager.fail_open is False
