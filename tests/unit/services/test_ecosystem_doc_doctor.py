...
from omnicrawler.services.doctor import _module_from_rel_path, check_ecosystem_doc  # noqa: PLC2701


def test_module_from_rel_path_mapping() -> None:
    assert _module_from_rel_path("core/site_aliases.py") == "omnicrawler.core.site_aliases"
    assert _module_from_rel_path("services/progress.py") == "omnicrawler.services.progress"
    assert _module_from_rel_path("convertx/") == "omnicrawler.convertx"
    assert _module_from_rel_path("fetching/async_fetcher") == "omnicrawler.fetching.async_fetcher"
    assert _module_from_rel_path("omnicrawler/quality/normalizers.py") == "omnicrawler.quality.normalizers"
...
def test_ecosystem_doc_file_exists() -> None:
    ...
    doc = Path(omnicrawler.__file__).resolve().parent.parent.parent / "docs" / "ECOSYSTEM_OBSERVATION.md"
...
def test_doc_fused_module_resolves_individually() -> None:
    ...
    doc = (
        Path(omnicrawler.__file__).resolve().parent.parent.parent
