...
市场安装不等于执行。普通用户应先打开目标项目，在“插件市场”中选中已安装插件，点击
“启用到当前项目”并确认精确权限。启用后宿主会立即重新加载插件，无需重启：声明了 `view`
的插件显示为工作台侧边面板，声明了 `resource_provider` 的插件在该面板中提供目录或资源选择。
如果启用失败，界面会保留项目配置并显示首条加载原因；修复签名、兼容性或授权问题后可重新授权。
需要暂时停用时，点击“在当前项目禁用”：宿主会立即卸载该插件的运行态并撤销当前项目授权，
但保留本机安装文件，也不影响其他项目。以后重新启用时必须再次确认权限。

插件启用、权限授权和载荷哈希均属于项目配置。插件更新或文件变化会使旧授权失效；复制安装目录、
手改 manifest 或只把插件放进 `plugins_installed/` 都不能替代正式启用。背景表面只作用于
OmniCrawler 工作台，不会直接修改操作系统桌面壁纸。
