...
def generate_omnicrawler_template(platform: str) -> str:
...
# 建议配合 omnicrawler auto-analyze 或 visual-select 自动生成选择器。
...
def generate_all_templates(output_dir: str) -> int:
    ...
    for platform in APIFY_ACTOR_MAP:
        template = generate_omnicrawler_template(platform)
        if template:
...
def main() -> None:
    ...
    if args.generate:
        template = generate_omnicrawler_template(args.generate)
        if template:
            print(template)
        else:
