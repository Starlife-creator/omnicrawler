...
class HeadlessRunner:
    ...
    def __init__(self, omnicrawler_path: str = "omnicrawler") -> None:
        """初始化无 GUI 执行器。
    ...
    def run(self, config_path: Path, log_level: str = "INFO") -> int:
        ...
        # 检查 omnicrawler
        available, version = check_omnicrawler(self._omnicrawler_path)
        if not available:
            print(_(f"\033[31m[ERROR]\033[0m omnicrawler 命令不可用 (路径: {self._omnicrawler_path})"),
                  file=sys.stderr)
            return 1
        ...
        try:
            ...
            process = subprocess.Popen(
                [self._omnicrawler_path, "run", "-c", str(config_path), "--log-level", log_level],
...
def run_headless(config_path: str, log_level: str = "INFO",
                 omnicrawler_path: str = "omnicrawler") -> int:
    """无 GUI 模式的便捷入口函数。
    ...
    Returns:
        退出码：0 成功，1 失败。
    """
    runner = HeadlessRunner(omnicrawler_path=omnicrawler_path)
