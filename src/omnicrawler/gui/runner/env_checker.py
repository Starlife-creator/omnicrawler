...
检测 omnicrawler 框架可用性、版本一致性和磁盘空间。
"""
...
class EnvCheckResult:
    ...
    def __init__(self) -> None:
        self.omnicrawler_available: bool = False
        self.omnicrawler_path: str = ""
        self.omnicrawler_version: str = ""
    ...
    def is_ok(self) -> bool:
        """环境是否就绪。"""
        return self.omnicrawler_available and len(self.errors) == 0


def check_omnicrawler(command_path: str = "omnicrawler") -> tuple[bool, str]:
    """检查 omnicrawler 命令是否可用。
    ...
    try:
        ...
        logger.warning(_("omnicrawler 探测超时（%ss），路径: %s"), timeout, resolved_command)
        if bundled is not None:
            return True, _("（内置引擎启动较慢）")
        return False, ""
    except FileNotFoundError as exc:
        logger.warning(_("omnicrawler 命令不存在: %s（%s）"), resolved_command, exc)
        return False, ""
    except OSError as exc:
        logger.warning(_("omnicrawler 探测失败（%s）: %s"), type(exc).__name__, exc)
        return False, ""
...
def run_full_check(
    omnicrawler_path: str = "omnicrawler",
    project_root: Path | None = None,
) -> EnvCheckResult:
    """运行完整环境检测。

    Args:
        omnicrawler_path: omnicrawler 命令路径。
        project_root: 项目根目录。
    ...
    omnicrawler_path = resolve_cli_command(omnicrawler_path)
    result.omnicrawler_path = omnicrawler_path

    # 检查 omnicrawler
    available, version = check_omnicrawler(omnicrawler_path)
    result.omnicrawler_available = available
    result.omnicrawler_version = version

    if not available:
        result.errors.append(
            _(f"无法执行 omnicrawler 命令（路径: {omnicrawler_path}）。") +
...
def try_auto_install(project_root: Path | None = None) -> tuple[bool, str]:
    ...
    if root is None:
        return False, _("未找到 OmniCrawler 项目根目录，请手动指定 omnicrawler 路径")
    ...
    try:
        ...
        if result.returncode == 0:
            # 验证安装
            available, version = check_omnicrawler()
            if available:
                return True, _(f"安装成功！omnicrawler {version}")
            return False, _("安装完成但 omnicrawler 命令仍不可用，请检查 PATH 环境变量")
        else:
