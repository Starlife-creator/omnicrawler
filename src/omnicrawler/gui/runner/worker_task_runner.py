...
def _derive_worker_command(omnicrawler_path: str) -> list[str] | None:
    """从用户指定的 CLI 路径推导配套 worker 可执行。

    F35：GUI「手动指定路径」所选可执行应对实际 worker 运行生效；
    仅当路径是真实文件且同目录存在 omnicrawler-worker.exe 时采用，
    否则回退 backend 自动探测（sys.executable 或冻结同目录 worker）。
    """
    if not omnicrawler_path or omnicrawler_path == "omnicrawler":
        return None
    candidate = Path(omnicrawler_path).expanduser()
    if not candidate.is_file():
        return None
    worker = candidate.parent / "omnicrawler-worker.exe"
    if worker.is_file():
        return [str(worker)]
    return None
...
class WorkerTaskRunner(QObject):
    ...
    def __init__(
        ...
        self._omnicrawler_path = omnicrawler_path
        self._project_root = Path(project_root) if project_root else Path.cwd()
        self._backend = LocalWorkerBackend(worker_command=_derive_worker_command(omnicrawler_path))
        self._state = "idle"
    ...
    def set_omnicrawler_path(self, command_path: str) -> None:
        if self.is_running:
            raise RuntimeError(_("任务运行期间不能切换 omnicrawler 命令"))
        self._omnicrawler_path = command_path
