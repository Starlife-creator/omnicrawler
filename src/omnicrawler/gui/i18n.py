...
翻译文件存放路径：locale/<lang>/LC_MESSAGES/omnicrawler-gui.mo
"""
...
# "omnicrawler-gui.pot" 不匹配——假语言包根因，切换语言永不生效）
...
def _find_localedir() -> Path | None:
    ...
    旧实现的三个候选（gui/locale、omnicrawler/locale 下两级）全都不存在——
    真实位置在仓库根的 ``locale/``（或打包后的 ``site-packages/omnicrawler/locale``），
    ...
    for ancestor in here.parents:  # gui → omnicrawler → src → 仓库根 → ...
        candidates.append(ancestor / "locale")
        candidates.append(ancestor / "locales")
    # pip wheel 形态：data-files 安装到 share/omnicrawler/locale
