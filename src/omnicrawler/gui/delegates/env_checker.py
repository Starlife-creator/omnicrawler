...
class EnvironmentChecker(_BaseDelegate):
    ...
    def _apply_probe_result(self, available: bool, version: str) -> None:
        mw = self._mw
        mw._omnicrawler_available = available
        if available:
            mw._settings.omnicrawler_version = version
    ...
    def _apply_install_result(self, ok: bool, result: str) -> None:
        ...
        if ok:
            mw._omnicrawler_available = True
            mw._omnicrawler_path = resolve_cli_command("omnicrawler")
            mw._settings.omnicrawler_path = mw._omnicrawler_path
            mw._task_runner.set_omnicrawler_path(mw._omnicrawler_path)
    ...
    def check_environment(self, silent: bool = True) -> bool:
        ...
        from ..runner.env_checker import check_omnicrawler
        available, version = check_omnicrawler(mw._omnicrawler_path)
        mw._omnicrawler_available = available
        if available:
            mw._settings.omnicrawler_version = version
            mw._settings.env_checked = True
    ...
    def recheck_env(self) -> None:
        ...
        if not mw._omnicrawler_available:
            QMessageBox.warning(mw, _("环境检测"), _("omnicrawler 命令不可用。请确保 OmniCrawler 框架已安装。"))
    ...
    def show_env_setup_dialog(self) -> None:
        ...
        from ..runner.env_checker import check_omnicrawler, try_auto_install
            ...
            if clicked == retry_btn:
                ...
                class _ProbeWorker(BackgroundWorker):
                    def work(self) -> tuple[bool, str]:
                        return check_omnicrawler(mw._omnicrawler_path)
        ...
        msg = QMessageBox(mw)
        msg.setWindowTitle(_("环境配置"))
        msg.setText(_("未检测到 omnicrawler 命令。请选择配置方式："))
        ...
        if clicked == auto_btn:
            ...
            filepath, _filter = QFileDialog.getOpenFileName(mw, _("选择 omnicrawler 可执行文件"),
                filter=_("可执行文件 (*.exe *.bat *.cmd);;所有文件 (*)"))
            if filepath:
                available, version = check_omnicrawler(filepath)
                if available:
                    mw._omnicrawler_available = True
                    mw._omnicrawler_path = filepath
                    mw._settings.omnicrawler_path = filepath
                    mw._settings.omnicrawler_version = version
                    mw._task_runner.set_omnicrawler_path(filepath)
