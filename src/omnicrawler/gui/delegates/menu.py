...
class MenuBuilder(_BaseDelegate):
    ...
    def setup(self) -> None:
        ...
        new_action.triggered.connect(mw._config_delegate.new_config)
        file_menu.addAction(new_action)

        open_action = QAction(_("打开配置(&O)..."), mw)
        open_action.setShortcut(QKeySequence.StandardKey.Open)
        open_action.triggered.connect(mw._config_delegate.open_config)
        file_menu.addAction(open_action)

        file_menu.addSeparator()
        mw._recent_menu = QMenu(_("最近文件"), mw)
        file_menu.addMenu(mw._recent_menu)
        mw._config_delegate.refresh_recent_menu()

        file_menu.addSeparator()
        save_action = QAction(_("保存(&S)"), mw)
        save_action.setShortcut(QKeySequence(mw._settings.shortcuts["save"]))
        save_action.triggered.connect(mw._config_delegate.save_config)
        file_menu.addAction(save_action)

        save_as_action = QAction(_("另存为..."), mw)
        save_as_action.triggered.connect(mw._config_delegate.save_config_as)
        file_menu.addAction(save_as_action)

        history_action = QAction(_("配置历史与恢复..."), mw)
        history_action.triggered.connect(mw._config_delegate.show_config_history)
        file_menu.addAction(history_action)

        file_menu.addSeparator()
        import_action = QAction(_("导入配置包..."), mw)
        import_action.triggered.connect(mw._config_delegate.import_config_package)
        file_menu.addAction(import_action)

        export_action = QAction(_("导出配置包..."), mw)
        export_action.triggered.connect(mw._config_delegate.export_config_package)
        ...
        recheck_action.triggered.connect(mw._env_checker.recheck_env)
        ...
        for theme_name, theme_id in [(_("明亮"), "light"), (_("黑暗"), "dark"), (_("跟随系统"), "system")]:
            ...
            theme_action.triggered.connect(lambda checked, t=theme_id: mw._theme_manager.set_theme(t))
        ...
        try:
            ...
            for label, theme_id in plugin_theme_labels():
                ...
                plugin_theme_action.triggered.connect(
                    lambda checked, t=theme_id: mw._theme_manager.set_theme(t)
                )
        ...
        for label, scale in ((_('紧凑 90%'), 90), (_('标准 100%'), 100), (_('大字体 125%'), 125), (_('特大字体 150%'), 150)):
            ...
            action.triggered.connect(lambda _checked=False, value=scale: mw._theme_manager.set_interface_scale(value))
        ...
        contrast_action.toggled.connect(lambda value: mw._theme_manager.set_accessibility_option("high_contrast", value))
        ...
        color_action.toggled.connect(lambda value: mw._theme_manager.set_accessibility_option("color_blind_friendly", value))
        ...
        motion_action.toggled.connect(lambda value: mw._theme_manager.set_accessibility_option("reduced_motion", value))
        ...
        dnd_action.toggled.connect(mw._theme_manager.toggle_dnd)
        ...
        about_action.triggered.connect(mw._help_dialogs.show_about)
        ...
        selector_help_action.triggered.connect(mw._help_dialogs.show_selector_help)
        help_menu.addAction(selector_help_action)
        quick_start_action = QAction(_("快速上手指南"), mw)
        quick_start_action.triggered.connect(mw._help_dialogs.show_quick_start)
        help_menu.addAction(quick_start_action)
        faq_action = QAction(_("常见问题"), mw)
        faq_action.triggered.connect(mw._help_dialogs.show_faq)
