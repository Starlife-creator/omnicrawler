...
Public API::
    ...
    from omnicrawler import AppConfig, Pipeline, StateStore
"""
...
def _setup_compat_aliases() -> None:
    """注册向后兼容模块别名，使 `from omnicrawler.config import AppConfig` 仍可用。
    ...
    for old_name, new_subpath in _DEPRECATED_MODULE_MAP.items():
        old_full = f"omnicrawler.{old_name}"
        if old_full not in sys.modules:
            try:
                module = importlib.import_module(f"omnicrawler.{new_subpath}")
...
class _CompatMetaFinder:
    """S4.1：拦截 `from omnicrawler.<旧名> import X` 形式的子模块导入，
    惰性重定向到新路径（import 语句不触发模块级 __getattr__）。"""

    def find_spec(self, fullname, path=None, target=None):
        if not fullname.startswith("omnicrawler."):
            return None
        short = fullname[len("omnicrawler."):]
        if "." in short:
            return None
        ...
        new_path = f"omnicrawler.{new_subpath}"
...
def __getattr__(name: str):
    ...
    例如 `from omnicrawler.config import AppConfig` 会自动重定向到
    ...
    if name in _DEPRECATED_MODULE_MAP:
        new_path = f"omnicrawler.{_DEPRECATED_MODULE_MAP[name]}"
        try:
            module = importlib.import_module(new_path)
    ...
    raise AttributeError(f"module 'omnicrawler' has no attribute {name!r}")


# S4.1：惰性兼容重定向——不再 eager 导入全部兼容模块（import omnicrawler 毫秒级）
import sys as _sys
