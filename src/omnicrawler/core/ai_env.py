...
消除历史遗留的三处互不覆盖 .env 拼接（cwd/、~/.omnicrawler/、project_root/）
与多套变量名分裂（OMNICRAWL_AI_* / PDFX_LLM_* / secret://env 解析）。

约定：
- 写入真源：``ai_env_path(project_root)`` —— 项目 .env（无项目时 ~/.omnicrawler/.env）。
- 读取优先级：``os.environ`` > 项目 .env > 当前目录 .env > 用户级 ~/.omnicrawler/.env。
...
def ai_env_path(project_root: str | Path | None = None) -> Path:
    ...
    return Path.home() / ".omnicrawler" / ".env"
...
def ai_env_candidates(project_root: str | Path | None = None) -> list[Path]:
    ...
    user_env = Path.home() / ".omnicrawler" / ".env"
...
def ai_config_sidecar_path(project_root: str | Path | None = None) -> Path:
    ...
    与 .env 同目录：项目态 ``<root>/ai_config.json``，用户态 ``~/.omnicrawler/ai_config.json``。
    """
