...
def resolve_cli_candidates(configured: str = "omnicrawler") -> tuple[str, list[str]]:
    ...
    if not is_frozen():
        ...
        for subdir in ("Scripts", "bin"):
            venv_cli = venv_root / subdir / f"omnicrawler{suffix}"
            if venv_cli.is_file():
...
def resolve_cli_command(configured: str = "omnicrawler") -> str:
