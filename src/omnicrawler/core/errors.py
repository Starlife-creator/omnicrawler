...
class ConfigParseError(ValueError, OmniCrawlError):
    ...
    suggestion = "配置文件存在语法错误；请检查 YAML 缩进、引号匹配和字段名拼写，或运行 omnicrawler validate。"
...
class TemplateValidationError(OmniCrawlError):
    ...
    suggestion = "模板校验失败；请检查模板占位符是否正确填充，或使用 omnicrawler templates validate 批量检查。"
