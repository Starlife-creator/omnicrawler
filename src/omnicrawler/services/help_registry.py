...
HELP_ENTRIES = {
    ...
    "source.pagination": _entry("source.pagination", "翻页与操作学习", "告诉系统如何取得下一批结果。", "漏掉翻页会只采到第一页。", "先试「学习点击/搜索/翻页」自动捕获；已知 API 参数名时，在「分页方式」里直接选『按页码』或『按游标』并填参数名，比猜更可靠。", "点击一次‘下一页’，系统捕获XHR中的page或cursor", "验证码和越权访问不会被绕过。游标形态要给出响应里的下一页字段（JSONPath），漏填会在运行前报错。", "手填不存在的page参数；或选了游标却漏填「下一页字段」（现在会直接报错，不再静默只采一批）。", "先自动识别；无法确认时保留浏览器方案。", "可能改变动作、API候选、请求体和范围；改分页参数会让已有试跑结果失效。", professional_note="可检查page/offset/cursor候选。", developer_note="编译为动作IR和API候选IR；GUI 的「分页方式」由 core.pagination 契约渲染。", keywords=("下一页", "cursor", "offset"), auto_action="learn_pagination"),
    ...
    "fields.value_source": _entry(
        "fields.value_source",
        "取值方式与属性",
        "决定一个字段的值取自哪里：条目里的子元素，还是条目元素自身；取文本，还是取属性。",
        "很多列表里要的那一列就是卡片本身带的链接或图片地址，而不是它里面的文字。",
        "先在「取值方式」列选一种：选子元素时填选择器（取它的文字或属性）；选元素自身属性时填属性名（如 href、src），属性可从下拉选也可手输自定义属性。",
        "链接列：取值方式=元素自身属性，属性=href",
        "选元素自身属性时必须填属性名；条目元素上没有该属性会取到空值（这时应改回子元素）。",
        "把属性名写进选择器（写成 href，而不是选择器留空+属性 href）——那会被当成元素名去找，静默取不到值。",
        "默认按子元素+选择器取值；选择器留空且未选方式时会提示先选取值方式。",
        "改变取值方式会改变交付出去的列内容，需要重新试跑确认。",
        professional_note="取值位置契约见 core/field_value_source.py（child / element / element_attr）；引擎规则是「attr 有值取属性、否则取文本」。",
        developer_note="GUI 的属性列落到 extract.fields.<name>.attr；位置由选择器与属性推导，不落盘。",
        keywords=("取值", "属性", "href", "元素自身", "链接"),
    ),
    "fields.item_selector": _entry("fields.item_selector", "列表项选择器", "指定‘一条记录’对应页面上的哪个容器。", "决定列表任务是一行一条、还是一整页只算一条。", "填卡片或行的 CSS 选择器（如 div.item）；不确定就留空，先试跑看结果。", "div.item、li.product、ul.products > li", "填错会采不到记录或只采到一条；留空则整页按单个对象处理。", "把整页容器当成单个字段填写，或改了它却不重新试跑。", "留空，按整页单对象处理。", "会改变‘一条记录’的边界，必须重新试跑。", professional_note="映射到 extract.item_selector。", developer_note="GUI 可编辑的透传键，与 extract.mode 同层。", keywords=("列表项", "容器", "item_selector", "卡片")),
    "fields.analyze_page": _entry("fields.analyze_page", "分析页面并填字段", "抓取入口网址，按页面结构推断列表容器与字段选择器。", "省去手写 CSS 选择器：新手也能得到与该页面真正匹配的字段。", "先填好入口网址，再点该按钮；结果只**追加**不覆盖，可在表格里继续改。", "列表页会填 div.item 之类的容器 + h2/span 等真实选择器", "需要联网抓一次页面；页面需要登录或有反爬时会失败（会如实报错）。", "对着需要登录的页面点它，误以为分析失败是程序问题。", "不自动执行；点按钮才发起，且沿用任务自身的出网策略。", "会新增/补充字段规则，可能让已有试跑结果失效（需重新试跑）。", professional_note="等价 CLI 的 analyze；分析走与 crawl 相同的守卫。", developer_note="GUI 侧是 PageAnalyzeWorker → analyze_page()。", keywords=("分析", "自动", "识别", "选择器", "列表项")),
    ...
    "yaml.editor": _entry("yaml.editor", "YAML 编辑器与单一编辑权", "以 YAML 源码形式查看与编辑任务配置。", "任务工作台与 YAML 是同一份配置的两种视图；为避免冲突，任一时刻只有一个入口可写。", "工作台顶部「查看 YAML」是只读源码视图；在侧栏 YAML 编辑器页修改后保存，工作台会自动同步。", "在侧栏编辑器修改 max_pages 后保存，工作台重新加载并保留你的草稿", "工作台有未提交修改时，外部 YAML 编辑会触发冲突提示：加载 YAML 覆盖或保留工作台修改。", "工作台和编辑器同时编辑同一字段导致互相覆盖。", "工作台修改回写配置，YAML 文件是持久事实。", "外部编辑会置为锁定态并要求二选一，避免双重事实来源。", keywords=("YAML", "源码", "编辑器", "冲突", "锁定")),
}
