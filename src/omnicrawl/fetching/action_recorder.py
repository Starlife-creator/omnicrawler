...
_RECORDER_SCRIPT = r"""
(() => {
  if (window.__omnicrawlRecorderInstalled) return;
  window.__omnicrawlRecorderInstalled = true;
  ...
  document.addEventListener('click', e => window.__omnicrawlEmit({type:'click', selector:selector(e.target)}), true);
  document.addEventListener('change', e => window.__omnicrawlEmit({
    type:'change', selector:selector(e.target), value:e.target.value || '',
    secret:e.target.type === 'password'
  }), true);
  document.addEventListener('keydown', e => {
    if (['Enter','Tab','Escape'].includes(e.key)) window.__omnicrawlEmit({type:'keydown', selector:selector(e.target), key:e.key});
  }, true);
  let lastScroll = 0;
  window.addEventListener('scroll', () => {
    if (Date.now() - lastScroll > 1000 && window.scrollY + window.innerHeight >= document.body.scrollHeight - 50) {
      lastScroll = Date.now(); window.__omnicrawlEmit({type:'scroll'});
    }
  }, true);
})();
"""
...
def record_with_playwright(url: str, output: Path, *, timeout_seconds: int = 300) -> dict[str, Any]:
    ...
    with sync_playwright() as playwright:
        ...
        page.expose_function("__omnicrawlEmit", sequence.add_event)
