...
def build_parser() -> argparse.ArgumentParser:
    ...
    plugins = sub.add_parser("plugins", help="列出已注册插件")
...
╚══════════════════════════════════════════════════╝
  ...
  快速开始:
    omnicrawl wizard              交互创建配置文件
    omnicrawl workbench           启动图形工作台
    omnicrawl init <名称>         从模板创建项目
      --template static_html

  常用命令:
    omnicrawl run    -c <配置>    运行采集任务
    omnicrawl status -c <配置>    查看任务状态
    omnicrawl doctor -c <配置>    诊断配置和环境
    omnicrawl templates list      浏览内置模板库
...
def _print_error_hint(exc: Exception) -> None:
    ...
    if isinstance(exc, FileNotFoundError):
        hints.append("请确认文件路径正确，或运行 omnicrawl init 创建新配置")
        ...
        if "seeds" in msg or "入口" in str(exc):
            ...
            hints.append("模板不存在；运行 omnicrawl templates list 查看可用模板")
        else:
            hints.append("请运行 omnicrawl doctor -c <配置> 检查配置有效性")
    elif isinstance(exc, PermissionError):
        hints.append("请确认目标网站允许自动访问，或检查网络代理设置")
    elif isinstance(exc, (ConnectionError, TimeoutError)):
        hints.append("网络连接失败；请检查目标网址是否可访问，或尝试降低并发数")
    else:
        hints.append("请运行 omnicrawl doctor 检查环境，或查看 OmniCrawler-用户指南.md")
    if hints:
...
def _wizard(output: Path) -> None:
    ...
    print()
    print("下一步建议操作:")
    print(f"  1. omnicrawl doctor  -c {output_path}     # 检查环境和配置")
    print(f"  2. omnicrawl sample  -c {output_path}     # 试跑 3 页验证")
    print(f"  3. omnicrawl run     -c {output_path}     # 正式采集")
    print()
    print("💡 提示: 如需更精准的字段配置，可以:")
    print(f"   omnicrawl auto-analyze {seed} -o {output_path}")
    print("   omnicrawl visual-select        # 在浏览器中可视化点选元素")
    print()
    _json({"created": str(output_path), "next": f"omnicrawl doctor -c {output_path}"})
