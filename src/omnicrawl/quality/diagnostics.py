...
用法:
    from omnicrawl.quality.diagnostics import diagnose, DiagnoseReport
...
_DIAGNOSE_RULES: list[tuple[str, ErrorCategory, str, str, str, str]] = [
    ...
    (r"No space left|disk.full|磁盘空间|ENOSPC", ErrorCategory.LOCAL_RESOURCE,
     "磁盘空间不足",
     "工作目录所在磁盘已满",
     "清理不必要的文件；扩展磁盘空间；运行 omnicrawl cleanup",
     "新数据无法写入，已采集数据完好"),
    ...
    (r"ModuleNotFoundError|ImportError|No module named", ErrorCategory.ENVIRONMENT,
     ...
     "运行 pip install omnicrawl-platform[full] 安装所有依赖",
     "对应功能不可用"),
...
def diagnose(error_text: str, context: dict[str, Any] | None = None) -> DiagnoseReport:
    ...
    return DiagnoseReport(
        ...
        cause="错误类型不在已知诊断规则中",
        action="运行 omnicrawl doctor 进行全面诊断；查看完整日志",
        data_impact="未知",
        retryable=False,
        auto_fix=None,
    ...
    if category == ErrorCategory.SELECTOR_NOT_FOUND:
        ...
        if url:
            return AutoFix(
                ...
                command=f"omnicrawl auto-analyze {url}",
                reversible=False,
            )
    return None
