"""SVG icon registry — theme-aware icon pipeline for the OmniCrawler GUI.

Replaces Unicode emoji-based icons with resolution-independent SVG paths
that adapt to the current theme via ``VisualTokens``.

Usage::

    from .icon_registry import IconRegistry

    save_icon = IconRegistry.icon("save", size=24)
    btn.setIcon(save_icon)

Icons are cached by (name, size, color_key) so repeated calls are cheap.
"""
...
_SVG_ICONS: dict[str, str] = {
    ...
    "open": """<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z"/></svg>""",
    "run": """<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="5 3 19 12 5 21 5 3"/></svg>""",
    "stop": """<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="6" y="6" width="12" height="12"/></svg>""",
    "pause": """<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="6" y1="4" x2="6" y2="20"/><line x1="18" y1="4" x2="18" y2="20"/></svg>""",
    ...
    "about": """<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="16" x2="12" y2="12"/><line x1="12" y1="8" x2="12.01" y2="8"/></svg>""",
    ...
    "wizard": """<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"/></svg>""",
    ...
    "template": """<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/></svg>""",
    ...
    "monitor_active": """<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M18 8A6 6 0 0 0 6 8c0 7-3 9-3 9h18s-3-2-3-9"/><path d="M13.73 21a2 2 0 0 1-3.46 0"/><circle cx="18" cy="4" r="3" fill="#d83b01" stroke="none"/></svg>""",
}


class IconRegistry:
    """Theme-aware SVG icon cache.
    ...
    def icon(name: str, *, size: int = 24, color: str = "primary") -> QIcon:
        """Return a :class:`QIcon` from the named SVG, colored for the current theme.

        Args:
            name: Icon key (e.g. ``"save"``, ``"run"``).
            size: Pixel dimensions (square).
            color: Token key — ``"primary"``, ``"muted"``, ``"text"``,
                   ``"danger"``, ``"success"``.

        Returns:
            A theme-colored QIcon. Returns an empty icon if *name* is unknown.

        Icons are cached by ``(name, size, color)``, so repeated
        lookups of the same params avoid re-rendering.
        """
    ...
    def _resolve_color(key: str) -> QColor:
        """Map a semantic color key to the current VisualTokens value."""
        try:
            from .design_system import ThemeManager
            tokens = ThemeManager.instance().tokens
        except Exception:
            return QColor("#176B87")
