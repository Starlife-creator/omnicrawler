...
HELP_ENTRIES = {
    ...
    "http.user_agent": _entry("http.user_agent", "用户代理", "发送给网站的身份标识字符串。", "网站据此识别你的工具和联系方式；方便管理员联系你。", "保留默认字段，只改邮箱和名称。", "OmniCrawler/2.1 (+contact: my@email.com)", "虚假UA可被视为恶意。", "不替换默认邮箱导致管理员无从联系。", "系统生成含版本和邮箱的默认UA。", "部分网站据此调整响应内容（如移动版）。", keywords=("UA", "身份", "邮箱")),
    ...
    "recovery": _entry("recovery", "恢复与重试", "从中断、崩溃或失败中恢复已采集的进度。", "长时间采集难免中断；恢复可避免从头开始。", "中断后运行 omnicrawl resume；失败页面用 retry-failed。", "断网后：omnicrawl resume -c config.yaml", "只恢复已保存到SQLite的进度。", "中断后直接重新 run 导致重复采集。", "SQLite WAL 自动保存进度。", "resume 复用已有原始响应；reprocess 只重做提取。", keywords=("恢复", "中断", "resume", "重试")),
    ...
    "export": _entry("export", "结果导出", "将数据库中的记录重新导出为文件。", "修改提取规则后可重新导出而不重新采集。", "omnicrawl export -c config.yaml；指定 --run-id 导出特定批次。", "修改字段规则后：omnicrawl reprocess -c config.yaml", "reprocess 需要原始归档完整。", "重新采集而不是用已有数据重新导出。", "每次 run 结束自动导出。", "手动导出可指定不同的格式和批次。", keywords=("导出", "export", "CSV", "Excel")),
