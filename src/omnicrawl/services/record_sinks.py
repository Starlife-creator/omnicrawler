        ...
        *,
        ...
        if connection is None:
            ...
            try:
                ...
                raise RuntimeError(
                    "PostgreSQL storage requires: pip install omnicrawl-platform[postgresql]"
                ) from exc
        ...
        *,
        ...
        if client is None:
            try:
                ...
                raise RuntimeError(
                    "OpenSearch storage requires: pip install omnicrawl-platform[search]"
                ) from exc
        ...
        try:
            if kind in {"postgres", "postgresql"}:
                sink: RecordSink = PostgreSQLRecordSink(
                    str(value.get("dsn", "")),
                    str(value.get("table", "omnicrawl_records")),
                    egress=egress,
                )
            elif kind in {"opensearch", "search"}:
                sink = OpenSearchRecordSink(
                    value.get("hosts", []), str(value.get("index", "omnicrawl-records")),
