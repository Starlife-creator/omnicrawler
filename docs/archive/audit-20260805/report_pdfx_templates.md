...
审查范围：`src/omnicrawl/pdfx/`（22 个文件）与 `src/omnicrawl/templates/`（7 个文件），并交叉核对 `gui/views/pdf_workbench.py`、`pipeline_ops/pdf_integration.py` 等调用方。
...
### [high] src/omnicrawl/pdfx/ocr.py:244-253, 355-359 - 多进程 OCR 无依赖预检，初始化失败导致整条管线崩溃
...
### [high] src/omnicrawl/pdfx/extraction.py:317 - LLM 客户端构造失败使整个 extract 阶段硬失败，无规则降级
...
### [high] src/omnicrawl/pdfx/ocr.py:128 - Tesseract 后端默认语言与内置模板不兼容，扫描件 OCR 全部失败
...
### [high] src/omnicrawl/pdfx/parser.py:199-234 + database.py:169 - 写事务横跨整个文档解析，并发 worker 长时间持锁
...
### [high] src/omnicrawl/pdfx/service.py:94-116 - 阶段隔离不完整 + 失败被 GUI 误报为“处理完成”
...
### [high] src/omnicrawl/pdfx/ingest.py:91-98 - path+size 去重跳过 SHA-256，同路径同大小文件更新后数据陈旧
...
### [high] src/omnicrawl/pdfx/config.py:106 + normalization.py:211-239 - 类型白名单与规范化逻辑不一致，boolean/entity/relationship 成死代码
...
### [medium] src/omnicrawl/pdfx/extraction.py:234-282 - 每条记录为全部字段写 NULL 行，长表数据膨胀
...
### [medium] src/omnicrawl/pdfx/extraction.py:333-334 + parser.py:148-152 - 一次性提交全部文档 Future，十万级内存峰值
...
### [medium] src/omnicrawl/pdfx/ocr.py:398-405, 451-458 - OCR 逐页 UPDATE+独立 commit，写库性能差
...
### [medium] src/omnicrawl/pdfx/ocr.py:416-430, 468-481 - 每文档 3 条 SQL 更新状态，N 文档 3N 查询

- 现状：OCR 完成后对每个受影响文档执行 2 次 COUNT + 1 次 UPDATE。
- 建议：改用一次 `GROUP BY doc_id` 汇总 pending/done 页数，再统一 UPDATE（或窗口函数一次性算 status）。

### [medium] src/omnicrawl/pdfx/ocr.py:224-241 - CPU 温度保护在 Windows 上无效
...
### [medium] src/omnicrawl/pdfx/ocr.py:262-288 - worker 返回元组与类型注解不符，doc_id/completed 为死数据
...
### [medium] src/omnicrawl/pdfx/parser.py:79-82 - parse_document 无任何调用方（死代码）

- 现状：`parse_document` 仅在定义处出现，全仓库无调用。
- 建议：删除或补单元测试并注明用途；其内部 `list(...)` 整文档收集与 D36 流式设计矛盾。

### [medium] src/omnicrawl/pdfx/desktop.py:11-23 - suite_root 无任何调用方（死代码）

- 现状：`suite_root` 全仓库无调用（`open_path` 被 services/workbench 使用）。
- 建议：删除或接入实际用途。

### [medium] src/omnicrawl/pdfx/parser.py:61 - 每页无条件执行 find_tables，大文档解析开销大
...
### [medium] src/omnicrawl/pdfx/parser.py:239-248 / llm.py:76-85 - 高 DPI 整页渲染无内存护栏
...
### [medium] src/omnicrawl/pdfx/review.py:23-34 - read_only 工作簿未关闭，文件句柄泄漏
...
### [medium] src/omnicrawl/pdfx/validation.py:59-62 - value_pattern/无效正则未在配置期校验，运行时崩溃整个抽取事务
...
### [medium] src/omnicrawl/pdfx/template_health.py:63-67 + template_monitor.py:58 - 快照文件损坏导致 observe 崩溃
...
### [medium] src/omnicrawl/templates/template_monitor.py:86-92 - observations.jsonl 并发追加无锁、历史无界增长
...
### [medium] src/omnicrawl/templates/template_health.py:126-154 - TemplatePack 导入非事务性

- 现状：import_pack 校验全部通过后逐个 `write_bytes`（150-153），中途写失败（磁盘满/权限）会留下部分文件。
- 建议：先全部写入临时目录再整体 rename，或失败时回滚已写文件。

### [medium] src/omnicrawl/templates/apify_templates.py:149-215 - 生成模板 schema 疑似与当前 DEFAULTS 不一致
...
### [medium] src/omnicrawl/gui/views/pdf_workbench.py:656-660 - 导出文件结构 dict/list 不匹配
...
### [medium] src/omnicrawl/gui/views/pdf_workbench.py:113-116, 697-700 - 取消后 UI 卡死在 running 且密钥未清理
...
### [medium] src/omnicrawl/pdfx/__init__.py:3 - 触发 omnicrawl 全量兼容别名注册，首包导入沉重

- 现状：`from .. import __version__` 会执行 `omnicrawl/__init__.py` 的 `_setup_compat_aliases()`（omnicrawl/__init__.py:192），逐个 importlib 加载几十个兼容子模块（含 services.workbench 等再引 pdfx）。
- 问题：任何 `import omnicrawl.pdfx.xxx` 都会连锁加载大量模块，启动慢且存在循环导入隐患（pdfx 加载一半时被 services.workbench 反向引入）。
- 建议：pdfx/__init__ 直接内联版本号或 `importlib.metadata.version`，避免触发根包；`_setup_compat_aliases` 改惰性/按需注册。

### [medium] src/omnicrawl/pdfx/config.py:280-296 - 目录重叠校验不完整
...
### [medium] src/omnicrawl/templates/template_catalog.py:160-168 - URL 正则匹配无超时/大小约束（ReDoS 面）
...
### [medium] src/omnicrawl/templates/template_catalog.py:285 - min_core_version 默认值不一致

- 现状：dataclass 默认 `"0.0.1"`（template_catalog.py:43），`_read_record` 缺省时却填 `"1.0.0"`。
- 问题：同一语义两套默认，版本判断结果随路径不同。
- 建议：统一为同一常量。

### [medium] src/omnicrawl/pdfx/cli.py:110-113 - doctor/validate 先强校验配置，配置损坏时无法出诊断
...
### [medium] src/omnicrawl/pdfx/service.py:61-66 - prepare_config 在阶段隔离之外

- 现状：`prepare_config`（load+validate+mkdir）在 try/except 之外，配置错误直接冒泡。
- 建议：将配置加载错误归入 results["error"]，与阶段隔离语义一致，GUI 可显示友好消息。

### [medium] src/omnicrawl/pipeline_ops/pdf_integration.py:69 - PDF 数量统计口径不一致
...
### [low] src/omnicrawl/pdfx/exporter.py:41-61 - 宽表查询表达式随字段数线性膨胀
...
### [low] src/omnicrawl/pdfx/database.py:132 - 用 assert 校验 PRAGMA

- 现状：`assert ... == 1`，`python -O` 下断言被剥离，静默失效。
- 建议：改为显式 raise。

### [low] src/omnicrawl/pdfx/database.py:194-206 - reset parse 不恢复 parse_dead
...
### [low] src/omnicrawl/pdfx/extraction.py:352-354 - summary 类型注解与实际返回不符

- 现状：注解 `dict[str, int]`，实际含 `"stopped": True`（bool）与 `"extraction_methods": dict`。
- 建议：改为 `dict[str, Any]`。

### [low] src/omnicrawl/pdfx/extraction.py:237 - required_names 在循环内重复计算

- 现状：`required_names = {spec.name ...}` 位于每记录循环内。
- 建议：提到循环外。

### [low] src/omnicrawl/pdfx/extraction.py:145 / retrieval.py:89 - 小问题集合

- 现状：`_observable_confidence` 的 `str(page_no).isdigit()` 防御可读性差；`retrieval.py:89` 循环内 `import json`。
- 建议：前者显式 None 判断，后者移到模块顶部。

### [low] src/omnicrawl/pdfx/normalization.py:78-101 - 纯月份/纯数字日期格式解析不全

- 现状：“2023-12”（无日）→ pattern1/2 不命中 → pattern3 输出 “2023”，丢掉月份；“2024.5” 同理。
- 建议：补充 `yyyy[-./]mm` 无日模式。

### [low] src/omnicrawl/pdfx/normalization.py:119-145 - 实体表模块级缓存不感知文件变更

- 现状：`_entity_cache` 按路径缓存后永不过期，同进程内 CSV 被修改不生效。
- 建议：带 mtime 缓存键。

### [low] src/omnicrawl/pdfx/config.py:155-160, 180-191 / normalization.py:164-177 - 项目根推断逻辑重复两处

- 现状：`_pdf_project_root` 与 load_config 的 base 解析、normalization.from_config 三处逻辑几乎一致。
- 建议：抽取公共函数。

### [low] src/omnicrawl/templates/template_catalog.py:100-102 - get() 多匹配取第一个

- 现状：按 stem/name 匹配多个时静默取第一个。
- 建议：歧义时返回 None 或记录冲突。

### [low] src/omnicrawl/templates/template_health.py:143 - 导入校验对非 UTF-8 payload 抛裸 UnicodeDecodeError

- 现状：`yaml.safe_load(payload.decode("utf-8"))` 解码失败时无上下文错误。
- 建议：捕获并给出文件名与原因。

### [low] src/omnicrawl/templates/template_health.py:116-123 - 导出包内文件名未校验 Windows 非法字符

- 现状：`templates/{template_id}.yaml`，template_id 若含 `:`/`?` 等，Windows 上导入写盘失败。
- 建议：export 前按 safe_filename 归一或拒绝。

### [low] src/omnicrawl/pdfx/text_export.py:71-73 - 同名文档依赖 doc_id 前 8 位区分

- 现状：不同目录同名文件 → 同 stem，靠 sha256 前 8 位（32 位空间）区分。
- 建议：用完整 doc_id 或冲突时自动后缀。

### [low] src/omnicrawl/pdfx/cli.py:40-46 - doctor 不检查 safe_regex 依赖的 regex 库

- 现状：doctor 只查 fitz/yaml/openpyxl + OCR 后端依赖。
- 建议：补充 `regex` 检查（超时保护依赖它）。

### [ux] src/omnicrawl/gui/views/pdf_workbench.py:667 - 完成 toast 用 `docs.get('ingested')`
