...
#### [high] src\omnicrawler\pipeline\_run.py:363 - reprocess_records 重新导出被幂等提交跳过，输出文件陈旧
...
#### [high] src\omnicrawler\runtime\auto_pilot.py:253-274 - 磁盘保护（run_state→pause）提案被丢弃且触发门控过窄
...
#### [high] src\omnicrawler\runtime\redis_frontier.py:23-42 - push 非事务批量写入 + 生成器消费导致计数错误与丢任务
...
#### [high] src\omnicrawler\runtime\redis_frontier.py:21,35 - 队列成员用完整 payload 而非 fingerprint 去重 + seen 集无限增长
...
#### [medium] src\omnicrawler\pipeline\_run.py:239 - 资源超限（status="failed"）后仍执行 PDF 阶段
...
#### [medium] src\omnicrawler\pipeline\_run.py:131-136 - 每个 URL 写入一条 stage_checkpoint，表无限增长
...
#### [medium] src\omnicrawler\pipeline\_run.py:175,137 - max_pages 只统计成功页，实际请求数可超预算
...
#### [medium] src\omnicrawler\pipeline\_run.py:95 - max_pages=0 被当作未传
...
#### [medium] src\omnicrawler\pipeline\exporters.py:150-153 - Parquet 导出把所有值强转 str，丢失类型
...
#### [medium] src\omnicrawler\pipeline\exporters.py:66-70,44-51 - 扁平化键与基础字段同名时互相覆盖
...
#### [medium] src\omnicrawler\pipeline\exporters.py:112-118 - openpyxl 缺失时静默跳过 xlsx 导出
...
#### [low] src\omnicrawler\pipeline\exporters.py:80 - JSONL 行同时包含 data_json 与嵌套 data/evidence 冗余键
...
#### [low] src\omnicrawler\pipeline\exporters.py:87 - 摘要统计假设 endpoints 可 int() 化
...
#### [low] src\omnicrawler\pipeline\_exports.py:95 - 假设 record_sinks.status() 恒含 recent_errors 键
...
#### [low] src\omnicrawler\pipeline\_exports.py:43 - 未完成导出冲突的报错缺少导出器名
...
#### [low] src\omnicrawler\pipeline\_extract.py:109 - transform_record 返回 None 会使下游 AttributeError
...
#### [low] src\omnicrawler\pipeline\_extract.py:184-185 - quality_threshold / unique_by 的类型假设脆弱
...
#### [low] src\omnicrawler\pipeline\_fetch.py:27-37 - 线程本地 fetcher 永不关闭（资源泄漏）
...
#### [low] src\omnicrawler\pipeline\_fetch.py:20-26 - browser fetcher 单例跨工作线程共享，线程安全假定
...
#### [low] src\omnicrawler\pipeline\_builders.py:56-62 - _processor 实例缓存无锁（潜在竞态）
...
#### [low] src\omnicrawler\pipeline\core.py:95-101 - executor 线程数固定为首次并发值
...
#### [low] src\omnicrawler\pipeline\_run.py:266-274 - KeyboardInterrupt 期间 drain() 再次触发中断会丢失收尾
...
#### [low] src\omnicrawler\pipeline\_run.py:413 - 循环体内冗余 import time
...
#### [medium] src\omnicrawler\pipeline_ops\preflight.py:108-118 - run_sample 复用固定 workspace，样本状态跨次累积
...
#### [low] src\omnicrawler\pipeline_ops\preflight.py:46 - 磁盘检查的目标目录在 workspace 不存在时退化到 root
...
#### [low] src\omnicrawler\pipeline_ops\pdf_integration.py:69 - 只扫描顶层 *.pdf，忽略子目录
...
#### [low] src\omnicrawler\pipeline_ops\pdf_integration.py:27-28 - 已存在 project.yaml 时不校验内容
- 现状: `if project_path.is_file(): return project_path, False`。
- 问题: 配置损坏/指向错误输入目录时静默复用，PDF 阶段可能空跑或报晦涩错误。
- 建议: 加载并校验一次 project 配置，失败给出明确提示。

#### [low] src\omnicrawler\pipeline_ops\provenance.py:25-35 - 清单无条件包含所有 run 的 PDF
...
#### [low] src\omnicrawler\pipeline_ops\provenance.py:39 - local_path 不做工作区约束
...
#### [low] src\omnicrawler\pipeline_ops\task_spec.py:74 - seeds 非列表时被逐字符迭代
...
#### [low] src\omnicrawler\pipeline_ops\plan_compiler.py:38 - seeds 用 str() 强转，非字符串元素产出垃圾 URL
...
#### [low] src\omnicrawler\pipeline_ops\plan_compiler.py:79-84 - plan_hash 基于脱敏配置，凭据变更不影响哈希
...
#### [low] src\omnicrawler\pipeline_ops\pipeline_stages.py:39 - 未知阶段名的报错来自 tuple.index
...
#### [high] src\omnicrawler\state\state_store.py:310-323 - claim 的 SELECT→UPDATE 非原子，多进程可双重认领
...
#### [medium] src\omnicrawler\state\state_store.py:158-161 - recover_incomplete_runs 无条件重置所有 in_progress
...
#### [medium] src\omnicrawler\state\state_store.py:254-261 - 增量周期 reset_all 把 blocked 也重置为 pending
...
#### [medium] src\omnicrawler\state\state_store.py:728-745 + src\omnicrawler\pipeline\_run.py:228-229 - 每处理一个 URL 就做一次全表聚合统计
...
#### [medium] src\omnicrawler\state\schema.py:190-201 - responses 表缺 final_url/url 索引
...
#### [medium] src\omnicrawler\state\state_store.py:631-651 - review_queue 全表扫描 + 逐行 JSON 解析
...
#### [low] src\omnicrawler\state\state_store.py:341-348 - attempts 跨周期/跨 resume 累积，重试预算失真
...
#### [low] src\omnicrawler\state\state_store.py:227-235 - fail_export 异常会掩盖原始导出异常
...
#### [low] src\omnicrawler\state\state_store.py:55-60 - close() 后调用任意方法会 AttributeError
...
#### [low] src\omnicrawler\state\state_store.py:447-449 - _preload_versions 临时表复用不 DROP
...
#### [low] src\omnicrawler\runtime\resources.py:42,54-62 - workspace 体积检查每间隔全目录 rglob 扫描
...
#### [low] src\omnicrawler\runtime\resources.py:32-33 - check 提前返回的 snapshot 缺 disk/workspace 键
...
#### [medium] src\omnicrawler\runtime\execution_backend.py:60-68 - InProcessBackend：service.run() 返回非 dict 时线程死亡、状态卡在 running
...
#### [medium] src\omnicrawler\runtime\worker_main.py:54-60 - worker 线程 run() 返回非 dict 时状态卡死（同因）
...
#### [medium] src\omnicrawler\runtime\execution_backend.py:209-214 - Windows 下 chmod 0600 不生效，会话 token 文件对本地用户可读
...
#### [low] src\omnicrawler\runtime\execution_backend.py:115-146 - session 文件重复 start 无锁、直接覆盖
...
#### [low] src\omnicrawler\runtime\execution_backend.py:148-156 - 启动握手超时但无任何异常时错误信息为空
...
#### [low] src\omnicrawler\runtime\worker_main.py:72-74 - shutdown 直接退出进程，运行中任务未优雅收尾
...
#### [medium] src\omnicrawler\runtime\scheduler.py:148-158 - 非法 allowed_hours 使 evaluate_conditions 抛 ValueError，schedule 被租约卡死
...
#### [medium] src\omnicrawler\runtime\schedule_conditions.py:11 - allowed_hours 用本地时间，next_run_at 用 epoch(UTC)，时区基准不一致
...
#### [low] src\omnicrawler\runtime\schedule_conditions.py:20-27 - 无电池的机器上 require_ac 被静默忽略
...
#### [low] src\omnicrawler\runtime\run_control.py:28-37 - 跨进程 read-modify-write 存在丢失更新
...
#### [low] src\omnicrawler\runtime\recovery.py:148-150 - 时间戳精度不足时隔离目录创建冲突
...
#### [low] src\omnicrawler\runtime\recovery.py:131,138 - next_command 中的 config 路径含空格未加引号
- 现状: `f"omnicrawler resume -c {self.config.path}"`。
- 问题: Windows 路径含空格时该命令无法直接复制执行（UX）。
- 建议: 用 `shlex.quote`（Windows 下按需加引号）。

#### [low] src\omnicrawler\runtime\recovery.py:124-131 - continue_incomplete 在无数据库时也创建 run_control.json
- 现状: 先 `RunControl(...).resume()` 再检查 `database.is_file()`。
- 问题: 无状态的任务工作区被“续跑”命令写入了控制文件，略显多余（副作用）。
- 建议: 先检查 DB 存在再写控制文件。

#### [medium] src\omnicrawler\runtime\scheduler.py:143-167 - run_due 串行阻塞执行，长任务拖住后续调度
...
#### [medium] src\omnicrawler\scheduling\change_detector.py:406 - 变更历史在内存中无限增长
...
#### [medium] src\omnicrawler\scheduling\change_detector.py:440-446 - save_rules 非原子写入，中断可能损坏规则文件
...
#### [low] src\omnicrawler\scheduling\change_detector.py:239,251 - 解析器回退使用过宽 except Exception
...
#### [low] src\omnicrawler\scheduling\change_detector.py:220-222 - node.text(strip=True) 被调用两次
- 现状: `"\n".join(node.text(strip=True) for node in nodes if node.text())`。
- 问题: 每节点解析两次，纯冗余（微性能）。
- 建议: 用辅助生成器先求值一次。

#### [low] src\omnicrawler\scheduling\change_detector.py:303-309 - 用户正则无复杂度约束（ReDoS 面）
...
#### [ux] src\omnicrawler\scheduling\change_detector.py:371-375 - 首次检查只建基线，contains/equals/regex 条件在首次绝不触发
...
#### [low] src\omnicrawler\state\schema.py:190-201 - 索引覆盖不足（汇总）
...
#### [low] src\omnicrawler\state\schema.py:46-64 - frontier 表无 run_id 归属
...
#### [medium] src\omnicrawler\state\state_store.py:263-275 - retry_failed 无上限默认一次性拉取全部失败行
- 现状: `LIMIT ?` 用 `1_000_000_000` 兜底并 `fetchall`。
- 问题: 大量失败行时一次性加载全部到内存并单条 executemany，内存/事务体积偏大。
- 建议: 分块（如 1000/批）处理。

#### [medium] src\omnicrawler\runtime\redis_frontier.py:44-49 - pop 不校验 seen 且依赖 payload 重建 CrawlRequest
...
#### [low] src\omnicrawler\runtime\adaptive_execution.py:35,56 - audit 列表无界增长
- 现状: `self.audit.extend(result)` 永不清空。
- 问题: 长期运行内存缓慢增长（与 AutoPilotState.audit 同理）。
- 建议: 限制保留条数或滚动落盘。

#### [low] src\omnicrawler\runtime\auto_pilot.py:293,299-326 - 通过私有属性 `_history._history` 访问
...
#### [low] src\omnicrawler\runtime\auto_pilot.py:60-76 - analyze 中 n<3 检查冗余（死代码）
- 现状: 开头 `len(self._history) < 3` 返回后，中间又 `if not values` 和 `if n < 3`。
- 问题: 后两个条件永远不成立（values 取最近 10 条，若历史≥3 则 values≥3）。
- 建议: 删除冗余分支。

#### [low] src\omnicrawler\state\state_store.py:257-261,298-299 - enqueue(force) 会把 blocked/done 直接打回 pending
...
#### [ux] src\omnicrawler\pipeline\_run.py:89 - setup 阶段失败摘要不含任何统计信息
- 现状: 失败摘要仅 `{"run_id", "status": "failed", "processed": 0, "error": str(exc)}`。
- 问题: 用户看不到已处理的页面/请求数，排查信息量不足。
- 建议: 附上 `self.state.stats(run_id)`（幂等读取）。

#### [ux] src\omnicrawler\pipeline\_exports.py:43 - 导出冲突错误文本（见前文 f-string 问题）另列 UX 影响
