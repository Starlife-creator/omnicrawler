...
- 环境探测：`import omnicrawler` 实测加载 273 个 omnicrawler.* 子模块，用时约 0.28–0.94s
- 范围：src/omnicrawler/{__init__,__main__,core,cli,commands,apps}（不含 pipeline/gui/pdfx 主体）
...
### [high] src/omnicrawler/__init__.py:147-159,192 - 包导入时急切导入全部 116 个兼容模块（重启动成本 + 副作用）

- 现状: `_setup_compat_aliases()` 在包加载末尾无条件执行，对 `_DEPRECATED_MODULE_MAP` 中全部 116 项逐一 `importlib.import_module(...)`；实测 `import omnicrawler` 经传递导入共加载 273 个 `omnicrawler.*` 子模块（services×28、runtime×13、fetching×12、extraction×8、plugins×8、templates×8 等），耗时 0.28–0.94s。
- 问题: 每个 CLI 命令启动都要 `from .. import __version__`（cli/_main.py:12），即每个 `omnicrawler xxx` 都要承受整包导入开销；任何被映射模块的顶层导入副作用都会在“仅 import 包”时执行；`except Exception: logger.debug(...)`（159 行）把所有失败静默吞掉（debug 级别默认不可见），掩盖真实导入错误。
- 建议: 删除 192 行的急切调用，仅保留 `__getattr__`（162-188 行）的按需重定向路径；或改为按需注册：第一次 `from omnicrawler.<old> import ...` 时再导入。

### [high] src/omnicrawler/core/ai_env.py:84-96 - load_ai_env 文件优先级颠倒（用户级 .env 反而覆盖项目级）

- 现状: `ai_env_candidates()`（50-61 行）按 [项目 .env, 当前目录 .env, 用户级 ~/.omnicrawler/.env] 返回（高→低）；`load_ai_env` 用 `for path in ai_env_candidates(...): merged.update(parse_env_file(path))`（91-92 行）顺序合并。
...
### [high] src/omnicrawler/core/utils.py:35-42 - deep_merge 对未覆盖的嵌套 dict 与原 DEFAULT 别名共享，会被运行时写入污染全局默认值

- 现状: `deep_merge` 只 `dict(base)` 浅拷贝顶层，未在 override 中出现的嵌套段（如 `crawl`）直接复用 `DEFAULTS["crawl"]` 同一对象。而 `ApplicationService.run`（services/application_service.py:89）会执行 `config.raw["crawl"]["max_pages"] = max_pages`，由 `omnicrawler run --max-pages N`（commands/run_task.py:62-67）触发。
...
### [high] src/omnicrawler/core/config.py:219-221 与 src/omnicrawler/cli/_handlers.py:405-409 - `omnicrawler validate` 的失败分支是死代码，JSON 契约失效
...
### [medium] src/omnicrawler/commands/template.py:97-105 - render 后的“渲染结果校验”是死代码，--force 放行分支永不触发
...
### [medium] src/omnicrawler/core/runtime_manifest.py:45 - verify_runtime_manifest 对畸形清单会抛未捕获 KeyError/TypeError
...
### [medium] src/omnicrawler/core/utils.py:45-63 - canonicalize_url 访问 parts.port 在 try 之外，畸形端口抛出未捕获 ValueError
...
### [medium] src/omnicrawler/core/archive_security.py:88-106 - copy_zip_member 出错留半写文件；已存在目标抛裸 FileExistsError
...
### [medium] src/omnicrawler/commands/run_status.py:47-49 与 src/omnicrawler/cli/_handlers.py:433-435 - `status --format text` 同时输出人类文本和完整 JSON
...
- 问题: `omnicrawler status --format text` 会先打印一段带 emoji 的可读文本，紧接着再打印整段 JSON——输出被污染，脚本与人都难受；`--format json` 时则正常。
- 建议: 让 handler 根据 `args.format` 决定只 `_json` 或只 `_print_text`（把打印职责收敛到 handler 一处），或让 `_print_text` 返回字符串供 handler 统一处理。

### [medium] src/omnicrawler/core/errors.py:59-82 - 定义了一整套类型化异常却从未被抛出
...
### [low] src/omnicrawler/core/credentials.py:11 - 环境变量前缀 OMNICRAW_SECRET_ 少了个 L，与全项目命名不一致
...
### [low] src/omnicrawler/core/ai_env.py:69,123 - .env 固定按 UTF-8 读取，GBK 编码文件抛未捕获 UnicodeDecodeError
...
### [low] src/omnicrawler/commands/schedule.py:13-15 - `_json` 辅助函数定义后从未使用

- 现状: 模块内 `_json` 定义存在，`execute` 全部路径返回 dict，由 `cli/_handlers._run_schedule` 负责序列化。
- 问题: 死代码。
- 建议: 删除。

### [low] src/omnicrawler/cli/_main.py:520-530 与 src/omnicrawler/commands/template.py:18-28 - `_key_values` 两处逐字重复

- 现状: 两个模块各自定义了一份完全相同的 `_key_values`（在 _main 中仅作为 `cli/__init__` 再导出被使用）。
- 问题: 重复代码，后续修改易漏改一处。
- 建议: 收敛到 `core.utils` 单一定义并复用。

### [low] src/omnicrawler/cli/_main.py:215 与 src/omnicrawler/cli/_handlers.py:173-180 - `import-easyspider --ir` 选项解析了但从未使用
...
### [low] src/omnicrawler/core/logging_utils.py:34 - configure_logging 对非法 level 字符串抛裸 AttributeError
...
### [low] src/omnicrawler/commands/field.py:12 - execute_field_suggest 默认 limit=20 与 CLI 默认 limit=100 不一致
...
### [low] src/omnicrawler/commands/init_project.py:30-35 - 项目名直接拼进输出文件名，未净化（可写出目标目录之外）

- 现状: `target = target_dir / f"{name}.yaml"`，`name` 是 CLI 位置参数，未做任何净化；随后 `data.setdefault("project", {})` 假定模板解析结果是 dict。
- 问题: `omnicrawler init "../../evil"` 之类的名称会把文件写到 `target_dir` 之外（本地文件系统穿越）；空/非对象模板会抛 AttributeError。
- 建议: 对 name 复用 wizard 中的净化规则（`_NAME_RE`），并对 `yaml.safe_load` 结果做 `isinstance(data, dict)` 校验。

### [low] src/omnicrawler/core/ai_env.py:99-105 - _format_env_line 不处理含换行的值，会写坏 .env

- 现状: `_QUOTE_CHARS` 不含 `\n`，值含换行时直接拼接成多行。
- 问题: 生成的 .env 被破坏，后续解析错位。
- 建议: 遇到 `\n`/`\r` 时替换为字面 `\n` 序列或拒绝写入并报错。

### [low] src/omnicrawler/core/runtime_paths.py:211-218 - bundled_browser_executable 仅匹配 Windows 路径，冻结版在 Linux/macOS 永远找不到 Chromium
...
### [low] src/omnicrawler/cli/_main.py:257-259,328-347 - pdf 子命令不注册到 argparse，`--help` 不显示、参数顺序受限

- 现状: `pdf`/`pdf-process`/`pdf-extract` 靠 `argv[0] in {...}` 前缀匹配分发，未加入 `build_parser`。
- 问题: `omnicrawler --help` 看不到这些命令；`omnicrawler --log-level DEBUG pdf ...` 因 argv[0] 不是 pdf 而不会分发（参数被 argparse 拒绝）。
- 建议: 将这些命令注册为隐藏 subparser 并在 handler 中转发，或明确提示 “可用 omnicrawler pdf --help”。

### [low] src/omnicrawler/core/capabilities.py:199-201 - all_optional_ready 对未导入模块默认乐观判定为可用
...
### [low] src/omnicrawler/commands/recovery.py:28 与 src/omnicrawler/commands/workspace.py:24 - --backup/--target 路径未做 expanduser/resolve 统一处理
...
### [low] src/omnicrawler/commands/__init__.py:10-13 - `__all__` 列出的子模块名未在包内 import

- 现状: `__all__` 声明 run_task/run_status/... 等模块名，但 `__init__` 并未导入它们。
- 问题: `from omnicrawler.commands import run_task` 依赖 Python 的子模块自动导入机制才成立，`import *` 与静态检查工具可能误判；属性访问时序不一致。
- 建议: 在 `__init__` 显式 `from . import run_task, ...` 或移除 `__all__`。

### [ux] src/omnicrawler/cli/_main.py:503-517 - wizard 结束同时输出人类文本块与机器 JSON 到 stdout
...
### [ux] src/omnicrawler/cli/_main.py:476-481 - wizard 输出格式输入未校验，非法值静默写入配置
...
### [ux] src/omnicrawler/cli/_handlers.py:120-125 - templates validate 存在双重退出码判断
