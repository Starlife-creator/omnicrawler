...
- 源码安装支持 Python 3.10+。
- 持续集成覆盖 Windows 与 Linux 的 Python 3.10、3.12、3.13。
- macOS 提供源码启动脚本；当前没有 macOS 持续集成验证，发布前不得将其描述为已验证的便携版平台。
...
| 可选 | Redis、Scrapy、S3、DuckDB、Parquet、PostgreSQL、OpenSearch | 按依赖安装后通过 `omnicrawl capabilities` 检查 |
| 显式例外 | 私网、代理、未拦截 Selenium、外部插件与外部 AI | 用户配置确认、出口审计与安全报告 |

## 质量与性能

- 全源码覆盖率门禁为 >= 50%；下一阶段目标为 >= 70%，安全、状态、管线、PDF/OCR、桌面核心另有分组门禁。
- 性能优化必须保留或提高基准任务的吞吐、延迟、内存和恢复效率；无法证明的回归不应合入。
- `omnicrawl capabilities --mode quick` 只检查核心运行条件；`--mode task --require browser` 等只导入当前任务所需组件；`--mode deep` 或兼容选项 `--verify-imports` 才导入全部已安装模块。三者均不把未安装的可选能力误报为可用。
...
- 便携版的最终可用能力仍以包内运行时完整性校验和 `omnicrawl capabilities` 输出为准。
