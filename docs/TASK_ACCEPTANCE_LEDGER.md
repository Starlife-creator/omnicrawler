...
| API→游标分页→增量 | 单对象与包装数组已有固定内存输入；游标/增量仍无完整正确答案 | 自动分析生成REST配置后按正式JSONPath及字段候选规则试跑；错误item_path与全空字段会被门禁拒绝 | 部分验证 | 固定游标API，核对停止、二次同步和重复请求 |
...
- 单对象JSON API已能生成REST配置；自动配置门禁已改为复用正式运行的JSONPath和字段候选
  规则。当前固定样例只证明分析与试跑一致，尚未证明游标停止和二次增量同步。
- 首任务自动回归不经真实点击与任务子进程；人工走查记录仍为空。
- 三平台打包脚本仍自行解析安装依赖，CI锁文件检查尚不能证明发布产物消费同一依赖解。
