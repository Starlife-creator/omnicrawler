...
- **OCR 文本的汉字间空格会原样进入 `text` 字段值**（2026-09-13 实测 → 同日**提供显式开关**）。
  ...
  **已按方案 C 处理（不改默认产出，只提供显式开关）**：字段级 `collapse_whitespace: true`
  ...
  - 仍未解决（也不打算用归一掩盖）：OCR 会把千分位逗号读成小数点（`12,345.67` → `12.345.67`）
    —— 那是识别错误，只能靠复核环节兜住。
