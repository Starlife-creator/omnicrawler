...
> 适用版本：0.13.1 · 对应阶段：第四阶段（完整体验与 UI）· 维护状态：现行
...
| 结果自带复现信息 | 他人能重建这次运行 | 源/生效配置指纹 + 档位参数 + 环境 + **输入快照** |
| 只有可用运行才成为基线 | 退化检测不能被零吞吐基线废掉 | `BenchmarkResult.usable` |
| **只有可比的两条记录才下退化结论** | 跨档位/跨环境/输入已变时，吞吐差异不说明我们的代码变慢 | `comparability()` 四维判定（W6.6） |

## 基线记录里有什么（W6.6）

每条记录都带下列四维；**缺任一维、或任一维不同 ⇒ 判为不可比**，此时 `compare_benchmark`
**不下退化结论**（`regression` 恒为 `False`），只给出 `incomparable_reasons` 供人判断：

| 维度 | 字段 | 为什么 |
|---|---|---|
| **场景** | `profile` + `profile_settings` | 档位定义负载；同一档位的参数被改过也不再同场景 |
| **输入快照** | `input_sha256` | 取该次运行**实际落库**的 `url` + `content_sha256` 逐行哈希。同一份配置两次运行可能面对**不同的页面内容**（站点更新、A/B、登录态），字节数相同并不代表输入相同 |
| **有效配置** | `effective_config_sha256` | 施加档位后真正执行的那份配置 |
| **依赖环境** | `environment` | 包版本 / Python / 平台 / 解释器；键值集合，**与元组顺序无关** |

**旧记录**（没有这些字段）一律判为不可比 —— 方案明确要求如此：拿无法描述的记录当基线，
得出的"退化"结论没有意义。CLI 会打印：

```
⚠ 与历史基线不可比，不下退化结论：输入快照缺失（旧记录或采集失败）
```
...
| **来源证据** | 采到的记录里有多少条指向任务真值声明的页面；未声明路径的任务才退化为非空检查 |
| **字段自报完整度** | 流水线自己在 `evidence._quality.completeness` 里报的完整度均值，并检查是否与实际字段存在性矛盾 |
| **额外/重复交付** | 真值之外的记录数，以及其中身份已存在的重复记录数；精确结构化任务必须均为 0 |

后两项共同防止“看起来有数据”被误判为正确：来源字符串非空但指错页面、完整记录之外
夹带错采、同一记录重复交付、自报完整却实际缺字段，都会使任务失败。
