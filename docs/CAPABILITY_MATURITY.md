...
> 本矩阵反映 OmniCrawler 0.8.0 的能力成熟度状态。
...
| 持久事件循环、批量 DB 操作、连接池化 | Stable | 2.2.0+ 优化：异步组件复用持久事件循环，DB 批量（executemany/preload/pipeline），S3/HTTP 连接池化 |
| BrowserAction + BrowserEngine Protocol | Stable | 2.2.0+ 统一浏览器操作协议（PlaywrightAdapter/SeleniumAdapter） |
| 共享重试配置 | Stable | 2.2.0+ 统一 parse_retry_config()，不在 http_client 和 async_fetcher 分别内联 |
...
| SDK run/query 与扩展协议 | Preview | 2.x 内按弃用期演进；stable 接口按语义化版本维护，删除前至少一个小版本弃用期 |
| 隔离插件、影子修复、自适应执行 | Preview | 人工批准、权限和边界不可跳过；插件在隔离沙箱/子进程中运行 |
| 远程 Worker/团队编排 | Reserved | 2.x 不提供服务端控制面 |
